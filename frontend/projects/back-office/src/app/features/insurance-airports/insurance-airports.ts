...
@Component({
  selector: 'app-insurance-airports-page',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, NzButtonModule, NzCardModule, NzInputModule, NzTableModule],
  templateUrl: './insurance-airports.html',
  styleUrl: './insurance-airports.scss'
})
export class InsuranceAirportsPage {
  ...
  async load(): Promise<void> {
    ...
    try {
      const response = await this.service.listAirports(this.pageIndex - 1, this.pageSize, this.search);
