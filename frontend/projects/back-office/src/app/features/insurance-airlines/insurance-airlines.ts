...
export class InsuranceAirlinesPage {
  ...
  async syncAirlines(): Promise<void> {
    this.isSyncing.set(true);
    try {
      await this.service.syncAirlines();
      this.message.success('Airlines synced successfully from Airlabs');
      await this.load();
    } catch (e) {
      this.message.error('Failed to sync airlines from Airlabs');
    } finally {
