...
@Component({
  selector: 'app-audit-auth-page',
  standalone: true,
  imports: [CommonModule, FormsModule, NzButtonModule, NzCardModule, NzInputModule, NzTableModule],
  templateUrl: './audit-auth.html',
  styleUrl: './audit-auth.scss'
})
export class AuditAuthPage {
  ...
  async load(): Promise<void> {
    ...
    try {
      const response = await this.service.listAuthAuditTrail(this.pageIndex - 1, this.pageSize, this.search);
